...
static int debug_mask = DEBUG_USER_STATE | DEBUG_SUSPEND;	//janged
...
//static int state; janged initial state??

static int state = 0x0;
...
abort:
	spin_lock_irqsave(&state_lock, irqflags);
//	if (state == SUSPEND_REQUESTED_AND_SUSPENDED)		//janged
	{
		printk("[KENEL] %s, %d \n", __FUNCTION__, __LINE__);
...
//janged
extern void set_start_sleep_state(int value);
static void late_resume(struct work_struct *work)
{
	struct early_suspend *pos;
	unsigned long irqflags;
