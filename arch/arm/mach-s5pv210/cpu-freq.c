...
int s5pv210_set_cpufreq_level(unsigned int mode)
{
	if (cpufreq_init == 0) {
		return -EPERM;
	}
	...
	case NORMAL_TABLE:
		...
		break;
	case RESTRICT_TABLE:
		if (S5PV210_FREQ_TAB == 0) {
			spin_unlock(&cpufreq_lock);
			return MAXIMUM_TABLE;
		}
		cpufreq_level_count++;
		S5PV210_FREQ_TAB = 2;
		break;
...
#if defined(CONFIG_S5PC110_MEM_DMC0_MDDR)
...
#else
		reg |= (0 << S5P_CLKDIV6_ONEDRAM_SHIFT);
#endif
		__raw_writel(reg, S5P_CLK_DIV6);
