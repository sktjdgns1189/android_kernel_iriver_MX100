...
static struct clksrc_clk clk_uclk1_133 = {
        .clk    = {
                .name           = "uclk1_133",
                .id             = -1,
        },
...
void __init_or_cpufreq s5pv210_setup_clocks(void)
{
...
#ifdef CONFIG_SERIAL_USE_HIGH_CLOCK
        clk_uclk1.rate = clk_get_rate(&clk_uclk1_133.clk);
#endif
	...
	/* JHLIM uart0 use High baudrate */
// ktj disable for bt sleep crash
//	clk_set_parent(&clk_mout_uart0.clk, &clk_mout_mpll.clk);  // BT
#ifndef CONFIG_SERIAL_USE_HIGH_CLOCK
	clk_set_parent(&clk_mout_uart1.clk, &clk_mout_mpll.clk);  // CDMA CM PORT

	{
		u32 val,shift=16,div = 1;
		u32 mask =  (0xffffffff >> (32 - 4)) << shift;

#if 0 // ktj disable for bt sleep crash
		val = __raw_readl(S5P_CLK_DIV4);
		val &= ~mask;
		val |= (div) << shift;
		__raw_writel(val, S5P_CLK_DIV4);  //uart0 bt 
#endif

		shift=20;
		mask =  (0xffffffff >> (32 - 4)) << shift;

		val = __raw_readl(S5P_CLK_DIV4);
		val &= ~mask;
		val |= (div) << shift;
		__raw_writel(val, S5P_CLK_DIV4);  // uart1 cdma cm port 
	}	
	//	s3c_set_clk_div(&clk_mout_uart0.clk,1);
	//	clk_set_rate(&clk_mout_uart0.clk, 200*MHZ);
