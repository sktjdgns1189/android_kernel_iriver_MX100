				    ...
				    struct mmc_card *card)
{
	...
	//if (dev && dev->id)
	//printk(" %d clock=%d type=%d \n", dev? dev->id:-1, ios?ios->clock:0, card?card->type:-1);
				   ...
				   void __iomem *r, int rw)
{
	...
	printk(" %s %x %x \n", __FUNCTION__,  pdata->rx_cfg, pdata->tx_cfg);
	writel(ctrl2, r + S3C_SDHCI_CONTROL2);
	writel(ctrl3, r + S3C_SDHCI_CONTROL3);
}
