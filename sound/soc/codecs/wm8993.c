...
static unsigned int wm8993_read_hw(struct snd_soc_codec *codec, u8 reg)
{
	...
	MX100_DBG(DBG_FILTER_WM8993_REG,"read reg %x %x\n",reg,(data >> 8) | ((data & 0xff) << 8));	
...
unsigned int wm8993_read(struct snd_soc_codec *codec,
				unsigned int reg)
{
	u16 *reg_cache = codec->reg_cache;

//	if(reg > WM8993_MAX_REGISTER) return 0;
...
int wm8993_write(struct snd_soc_codec *codec, unsigned int reg,
			unsigned int value)
{
	u16 *reg_cache = codec->reg_cache;
	u8 data[3];
	...
	if (ret == 3) {
		MX100_DBG(DBG_FILTER_WM8993_REG,"write reg  %d(0x%x) %x\n",reg,reg,value);	
		return 0;
	}
	if (ret < 0)  {
		MX100_DBG(DBG_FILTER_WM8993_REG,"wm8993 write error %x %x\n",reg,value);	
		return ret;
	}
		       ...
		       unsigned int Fout)
{
	...
	while ((Fref / div) > 13500000) {
		...
		if (div > 8) {
			MX100_DBG(DBG_FILTER_WM8993_REG,"Can't scale %dMHz input down to <=13.5MHz\n",
			       Fref);
			return -EINVAL;
		}
	}

	MX100_DBG(DBG_FILTER_WM8993,"Fref=%u Fout=%u\n", Fref, Fout);
	...
	while (target < 90000000) {
		...
		target *= 2;
		if (div > 7) {
			MX100_DBG(DBG_FILTER_WM8993_REG,"Unable to find FLL_OUTDIV for Fout=%uHz\n",
			       Fout);
			return -EINVAL;
		}
	}
	...
	MX100_DBG(DBG_FILTER_WM8993,"Fvco=%dHz\n", target);
	...
	if (i == ARRAY_SIZE(fll_fratios)) {
		MX100_DBG(DBG_FILTER_WM8993_REG,"Unable to find FLL_FRATIO for Fref=%uHz\n", Fref);
		return -EINVAL;
	}
	...
	fll_div->n = Ndiv;
	Nmod = target % Fref;
	MX100_DBG(DBG_FILTER_WM8993,"Nmod=%d\n", Nmod);
	...
	MX100_DBG(DBG_FILTER_WM8993,"N=%x K=%x FLL_FRATIO=%x FLL_OUTDIV=%x FLL_CLK_REF_DIV=%x\n",
		 fll_div->n, fll_div->k,
...
//		dev_dbg(codec->dev, "Using %dHz MCLK\n", wm8993->mclk_rate);
		MX100_DBG(DBG_FILTER_WM8993_REG,"Using %dHz MCLK\n", wm8993->mclk_rate);
	...
	//	} else 
		...
		wm8993_write(codec, WM8993_CLOCKING_2, reg); //jhlim
		break;
...
static int wm8993_route_get_mic_path(struct snd_kcontrol *kcontrol,
	struct snd_ctl_elem_value *ucontrol)
{
	MX100_DBG(DBG_FILTER_WM8993_MIXER,"wm8993_route_get_mic_path");

	return 0;
}

static int wm8993_route_set_mic_path(struct snd_kcontrol *kcontrol,
	struct snd_ctl_elem_value *ucontrol)
{
	#if 0
	...
	MX100_DBG(DBG_FILTER_WM8993_MIXER,"set_rec_path param %d", path_num);
...
SOC_ENUM_EXT("MIC Path", path_control_enum[0],
            wm8993_route_get_mic_path, wm8993_route_set_mic_path),
...
#if 0 //for remove warning message.
...
#else
SND_SOC_DAPM_AIF_OUT("mx100", "Capture", 0, SND_SOC_NOPM, 0, 0),
SND_SOC_DAPM_AIF_IN("mx100", "Playback", 0, SND_SOC_NOPM, 0, 0),
#endif
				 ...
				 enum snd_soc_bias_level level)
{
	...
	MX100_DBG(DBG_FILTER_WM8993_ALSA_ROUTE,"wm8993_set_bias_level %d\n",level);  //jhlim
			     ...
			     int clk_id, unsigned int freq, int dir)
{
	...
	default:
		MX100_DBG(DBG_FILTER_WM8993_REG,"ERROR!!! %d\n",__LINE__);  //jhlim
			    ...
			    struct snd_soc_dai *dai)
{
	...
	MX100_DBG(DBG_FILTER_WM8993_REG,"wm8993_hw_params\n");  // jhlim
...
//	best = 1; // JHLIM : 128  2011.02.05 44.1khz fix.
	...
	MX100_DBG(DBG_FILTER_WM8993_MIXER,"wm8993_hw_params  %d\n",substream->stream);  // jhlim
...
static int wm8993_digital_mute(struct snd_soc_dai *codec_dai, int mute)
{
	...
	if (mute) {
		msleep(10);
		mx100_wm8993_mute(MUTE_ON,WMS_DAC | WMS_DACLR);
		MX100_DBG(DBG_FILTER_WM8993_ALSA_ROUTE,"wm8993_digital_mute  on\n");
	}
	else {
		reg = wm8993_read(codec, WM8993_DAC_CTRL);
		reg |= (WM8993_DAC_OSR128);
		wm8993_write(codec, WM8993_DAC_CTRL, reg);
		MX100_DBG(DBG_FILTER_WM8993_ALSA_ROUTE,"wm8993_digital_mute  off\n");
		mx100_wm8993_mute(MUTE_OFF,WMS_DAC | WMS_DACLR);
		msleep(10);
	}
}
#endif
...
struct snd_pcm_substream *g_fm_substream = 0;
#endif

static int wm8993_startup(struct snd_pcm_substream *substream, struct snd_soc_dai *codec_dai)
{
	struct snd_soc_codec *codec = codec_dai->codec;
	struct wm8993_priv *wm8993 = codec->private_data;
#if 0
	MX100_DBG(DBG_FILTER_WM8993_ALSA_ROUTE,"wm8993_startup Stream_type = %d, Stream_state = [0x%X],  Codec State = [0x%X]\n"
		, substream->stream
		, wm8993->stream_state
		, wm8993->codec_state);
#endif
	...
	if(substream->stream == SNDRV_PCM_STREAM_PLAYBACK) {
		wm8993->stream_state |=  PCM_STREAM_PLAYBACK;
		#ifdef CONFIG_I2S_FM_FEED_MCLK		
		g_fm_substream = substream;
		#endif	
	}
	else {
		wm8993->stream_state |= PCM_STREAM_CAPTURE;
	}


//	mx100_wm8993_power(POWER_ON,WPS_MAIN);
	
	return 0;
}

void wm8993_shutdown(struct snd_pcm_substream *substream, struct snd_soc_dai *codec_dai)
{
	struct snd_soc_codec *codec = codec_dai->codec;
	struct wm8993_priv *wm8993 = codec->private_data;

#if 0
	MX100_DBG(DBG_FILTER_WM8993_ALSA_ROUTE,"wm8993_shutdown Stream_type = %d, Stream_state = [0x%X],  Codec State = [0x%X]\n"
		, substream->stream
		, wm8993->stream_state
		, wm8993->codec_state);
#endif
	...
	if(wm8993->stream_state & (PCM_STREAM_CAPTURE | PCM_STREAM_PLAYBACK) ){

	} else {
//		mx100_wm8993_power(POWER_OFF,WPS_MAIN);
	}
//	msleep(300);
}
...
static struct snd_soc_dai_ops wm8993_ops = {
	 .startup = wm8993_startup,
	 .shutdown = wm8993_shutdown,
...
struct mutex g_codec_ctl_mutex;
...
//	MX100_DBG(DBG_FILTER_WM8993_ALSA_ROUTE,"check tick noise this point ***");	
	if (ret < 0) {
...
void power_off_wm8993(void)
{
	printk("wm8993 power off\n");
	mx100_wm8993_mute(MUTE_ON,MWS_MUTE_ALL);
	msleep(300);
//	mx100_wm8993_power(POWER_OFF,WPS_POWER_ALL);
//	msleep(300);
}
...
#ifdef ENABLE_WM8993_PM
static u16 g_power_val0,g_power_val1,g_power_val2;
/* 2011.04.29 JHLIM */
void mx100_wm8993_power_save(void)
{
	...
	g_power_val0 = wm8993_read(codec, WM8993_POWER_MANAGEMENT_1);
	g_power_val1 = wm8993_read(codec, WM8993_POWER_MANAGEMENT_2);
	g_power_val2 = wm8993_read(codec, WM8993_POWER_MANAGEMENT_3);

	MX100_DBG(DBG_FILTER_PM,"wm8993 save:%x %x %x\n",g_power_val0,g_power_val1,g_power_val2);
	
}
void mx100_wm8993_power_restore(void)
{
	...
	wm8993_write(codec, WM8993_POWER_MANAGEMENT_1,g_power_val0);
	wm8993_write(codec, WM8993_POWER_MANAGEMENT_2,g_power_val1);
	wm8993_write(codec, WM8993_POWER_MANAGEMENT_3,g_power_val2);

	MX100_DBG(DBG_FILTER_PM,"wm8993 restore:%x %x %x\n",g_power_val0,g_power_val1,g_power_val2);
	
}

// added 2011.03.18 JHLIM
static int wm8993_suspend(struct platform_device *pdev,pm_message_t msg )
{
	struct snd_soc_codec *codec = wm8993_codec;
	struct wm8993_priv *wm8993 = codec->private_data;

	audio_control_lock();

//	if(wm8993->power_state == CODEC_ON)
	{
		wm8993->power_state = CODEC_OFF;

	#if 0	
		if(wm8993_route_get_playback_path()==HP3P || wm8993_route_get_playback_path()==HP4P	) {			
			mx100_wm8993_mute(MUTE_ON,WMS_HPOUT | WMS_ANALOGUE_HP |WMS_MIXOUT);
		} else if(wm8993_route_get_playback_path()==SPK)  {
			mx100_wm8993_mute(MUTE_ON,WMS_SPKOUT |WMS_MIXOUT);
		}
	#else
		wm8993_setpath_mic_off();
		mx100_wm8993_mute(MUTE_ON,WMS_MIXOUT);
		mx100_hp_hw_mute(1);
		msleep(100);
	#endif
		mx100_wm8993_power_save();	
		msleep(100);
		mx100_wm8993_power(POWER_OFF,	WPS_CAPTURE);
		mx100_wm8993_power(POWER_OFF,	WPS_MIXIN);
		mx100_wm8993_power(POWER_OFF,	WPS_MIXOUT);
		...
		reset_4pole_sendkey_ready();

		printk("wm8993_suspend\n");
	}
...
//	if(wm8993->power_state == CODEC_OFF)
	{
		// Turn on sequence by recommend Wolfson.
		audio_control_lock();
		wm8993->power_state = CODEC_RESUME;
		reset_4pole_sendkey_ready();

		mx100_wm8993_power(POWER_ON,WPS_MAIN);
		mx100_wm8993_power_restore();
		mx100_wm8993_power(POWER_ON,WPS_MAIN);

		msleep(100);
		#if 1
		mx100_wm8993_mute(MUTE_OFF,WMS_MIXOUT);
		mx100_hp_hw_mute(0);
		msleep(100);
		#else
		if(wm8993_route_get_playback_path()==HP3P || wm8993_route_get_playback_path()==HP4P	) {
			...
			mx100_wm8993_mute(MUTE_OFF,WMS_SPKOUT |WMS_MIXOUT);
		}
		#endif
		
		printk("wm8993_resume\n");
		
		audio_control_unlock();
		
		wm8993->power_state = CODEC_ON;
...
//	int i;
	...
	MX100_DBG(DBG_FILTER_WM8993_REG,"wm8993_i2c_probe\n");

	gpio_request(GPIO_EARJACK_MUTE_OUT, "GPIO_EARJACK_MUTE_OUT");
        gpio_direction_output(GPIO_EARJACK_MUTE_OUT,0);
        s3c_gpio_setpull(GPIO_EARJACK_MUTE_OUT, S3C_GPIO_PULL_DOWN);	
...
//	mx100_hp_hw_mute(1);
	...
	if (ret != 0)
		goto err;
	#if 0
	for(i=1;i< WM8993_REGISTER_COUNT;i++) {
		wm8993_write(codec, i, wm8993_reg_defaults[i]);
	}
	#endif
	...
	snd_soc_update_bits(codec, WM8993_RIGHT_DAC_DIGITAL_VOLUME/2,
			    WM8993_DAC_VU/2, WM8993_DAC_VU);
	...
	snd_soc_update_bits(codec, WM8993_CLOCKING_4, WM8993_SR_MODE, 0); // JHLIM 2011.02.06
//	snd_soc_update_bits(codec, WM8993_CLOCKING_4, WM8993_SR_MODE, 1); 
	...
	wm8993_write(codec, WM8993_SPEAKER_MIXER, 0b0000000100000000);

	val = wm8993_read(codec, WM8993_DAC_CTRL);
	val |= WM8993_DAC_MUTE;
	wm8993_write(codec, WM8993_DAC_CTRL, val);

	wm8993->power_state = CODEC_ON;

//	wm8993_set_audio_path(WAP_PLAY,1);
//	wm8993_set_audio_path(WAP_HEADSET,1);
//	wm8993_set_audio_path(WAP_MIC,1);
...
//	mx100_hp_hw_mute(0);
	...
	if(is_4pole_headset_connected()==0) {
...
#ifdef CONFIG_PM /* ktj */
static int wm8993_i2c_suspend(struct i2c_client *i2c, pm_message_t msg)
{
	return snd_soc_suspend_device(&i2c->dev);
}

static int wm8993_i2c_resume(struct i2c_client *i2c)
{
	return snd_soc_resume_device(&i2c->dev);
}
#else
#define wm8993_i2c_suspend  NULL
#define wm8993_i2c_resume   NULL
#endif
...
static struct i2c_driver wm8993_i2c_driver = {
    ...
    /* ktj, add suspend & resume */
	.suspend =  wm8993_i2c_suspend,
	.resume =   wm8993_i2c_resume,
...
static int __init wm8993_modinit(void)
{
	...
	if (ret != 0)
		MX100_DBG(DBG_FILTER_WM8993_REG,"WM8993: Unable to register I2C driver: %d\n", ret);
