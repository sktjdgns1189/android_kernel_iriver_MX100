...
#include "s3c64xx-i2s.h"
 ...
 #include <sound/wm8993.h> /* JHLIM */

#define REDUCE_I2S_POPS  /*JHLIM 2011.04.14  */
...
int tx_clk_enabled = 0;
int rx_clk_enabled = 0;

// added 2011.03.28 JHLIM : fm radio use i2s mclk.

#ifdef CONFIG_I2S_FM_FEED_MCLK
int fm_clk_enabled = 0;
#endif
...
//#define USE_DELAY_WORK_SHUTDOWN_I2S  /* JHLIM  reduce pops */

#ifdef USE_DELAY_WORK_SHUTDOWN_I2S
static struct delayed_work g_delayed_work_shutdown_i2s ;   /* JHL  */
static struct snd_soc_dai *g_cpu_dai;
#endif
...
void s5p_i2s_do_suspend(struct snd_soc_dai *dai)
{
	...
	if (!audio_clk_gated) {	 		/* Clk/Pwr is alive? */
		...
		#ifdef USE_DELAY_WORK_SHUTDOWN_I2S
		g_cpu_dai = dai;
		schedule_delayed_work(&g_delayed_work_shutdown_i2s,msecs_to_jiffies(700));
		#else
...
#ifdef USE_DELAY_WORK_SHUTDOWN_I2S
static void s5p_i2s_set_clk_enabled_delayed_work(struct work_struct *work)
{
	s5p_i2s_set_clk_enabled(g_cpu_dai, 0);	/* Gating Clk/Pwr */
}
#endif
...
void display_mute_status(int all_mute_state)
{	
	{
		int n=0;
		char szMuteStatus[128];
		n+= sprintf(&szMuteStatus[n],"MUTE STATUS:%d ",all_mute_state);
		
		if(all_mute_state & WMS_DAC) n+= sprintf(&szMuteStatus[n],"DAC ");
		if(all_mute_state & WMS_DACLR) n+= sprintf(&szMuteStatus[n],"DACLR ");
		if(all_mute_state & WMS_MIC) n+= sprintf(&szMuteStatus[n],"MIC ");
		if(all_mute_state & WMS_EXT_MIC) n+= sprintf(&szMuteStatus[n],"EXT_MIC "); 
		if(all_mute_state & WMS_FM) n+= sprintf(&szMuteStatus[n],"FM ");
		if(all_mute_state & WMS_HPOUT) n+= sprintf(&szMuteStatus[n],"HPOUT ");
		if(all_mute_state & WMS_MIXOUT) n+= sprintf(&szMuteStatus[n],"MIXOUT ");
		if(all_mute_state & WMS_SPKOUT) n+= sprintf(&szMuteStatus[n],"SPKOUT ");
		if(all_mute_state & WMS_ANALOGUE_HP) n+= sprintf(&szMuteStatus[n],"ANALOGUE_HP ");
		n+= sprintf(&szMuteStatus[n],"\n");
		MX100_DBG(DBG_FILTER_WM8993_POP,szMuteStatus);
		
		
	}
}

static int g_i2s_delay_startup_play1 = 50;
static int g_i2s_delay_startup_rec1 = 50;

static int g_i2s_delay_startup_play2 = 10;  // 0
static int g_i2s_delay_startup_rec2 = 10;  // 0

static int g_i2s_delay_shutdown_play1 = 50;  // 4    10
static int g_i2s_delay_shutdown_rec1 = 50;  // 10

static int g_i2s_delay_shutdown_play2 = 120;  //6 10 
static int g_i2s_delay_shutdown_rec2 = 70;

void set_i2s_delay(int index,int value)
{
	mx100_printk("set_i2s_delay%d  %d",index,value);
	switch(index) {
		case 0:  g_i2s_delay_startup_play1 = value;  break;
		case 1:  g_i2s_delay_startup_rec1 = value;  break;

		case 2: g_i2s_delay_startup_play2 = value;  break;
		case 3:  g_i2s_delay_startup_rec2 = value;  break;

		 case 4: g_i2s_delay_shutdown_play1 = value;  break;
		 case 5: g_i2s_delay_shutdown_rec1 = value;  break;

		case 6:  g_i2s_delay_shutdown_play2 = value; break;
		case 7:  g_i2s_delay_shutdown_rec2 = value; break;
	}
				...
				struct snd_soc_dai *dai)
{
	...
	/* 2011.04.13 JHLIM  */
	#ifdef REDUCE_I2S_POPS
	int mute_act = 0;
//	int mute_status = 0;

	if(fm_clk_enabled==0 && tx_clk_enabled == 0 && rx_clk_enabled == 0) {
		{
			mute_act = 1;
			if(wm8993_route_get_playback_path()==HP3P || wm8993_route_get_playback_path()==HP4P	) {
				...
				MX100_DBG(DBG_FILTER_WM8993_POP,"HP\n");
			} else if(wm8993_route_get_playback_path()==SPK) {
				mx100_wm8993_mute(MUTE_ON,WMS_MIXOUT |WMS_SPKOUT);
				MX100_DBG(DBG_FILTER_WM8993_POP,"SPK\n");
			}
			if (substream->stream == SNDRV_PCM_STREAM_PLAYBACK)  {
				msleep(g_i2s_delay_startup_play1);
			} else if(substream->stream == SNDRV_PCM_STREAM_CAPTURE) {
				msleep(g_i2s_delay_startup_rec1);
			}
		}
	}
	#endif
		...
		if(mute_act==1) {
			if (substream->stream == SNDRV_PCM_STREAM_PLAYBACK)  {
				msleep(g_i2s_delay_startup_play2);
			} else if(substream->stream == SNDRV_PCM_STREAM_CAPTURE) {
				msleep(g_i2s_delay_startup_rec2);
			}
...
#ifdef CONFIG_I2S_FM_FEED_MCLK
int s5p_i2s_fm_wr_startup(struct snd_pcm_substream *substream,
				struct snd_soc_dai *dai)
{
	 fm_clk_enabled = 1;

	if (tx_clk_enabled || rx_clk_enabled )  return 0;


	msleep(50);  // JHL reduce pops

	s5p_i2s_do_resume(dai);


	MX100_DBG(DBG_FILTER_I2S_FM_MCLK,"s5p_i2s_fm_wr_startup \n");

#ifdef CONFIG_S5P_LPAUDIO
	s5p_i2s_startup(substream, dai);
#endif

	msleep(100);  // JHL reduce pops		

	return 0;
}

void s5p_i2s_fm_wr_shutdown(struct snd_pcm_substream *substream, struct snd_soc_dai *dai)
{
	fm_clk_enabled = 0;

	if (!tx_clk_enabled && !rx_clk_enabled && !fm_clk_enabled) {	/* Tx/Rx/fm both off? */
		MX100_DBG(DBG_FILTER_I2S_FM_MCLK,"s5p_i2s_fm_wr_shutdown \n");
		#ifdef REDUCE_I2S_POPS
		/* 2011.04.13 JHLIM  */
		msleep(50);  // JHL reduce pops
		#endif
		s5p_i2s_do_suspend(dai);
		msleep(150);  // JHL reduce pops		
	}
	return;
}
...
//	int mute_status = 0;
	...
	if (tx_clk_enabled==0 && rx_clk_enabled==0 && fm_clk_enabled==0) {	/* Tx/Rx both off? */
		//MX100_DBG(DBG_FILTER_I2S_FM_MCLK,"s5p_i2s_wr_shutdown : fm_clk_enabled:%d",fm_clk_enabled);
		#ifdef REDUCE_I2S_POPS
		/* 2011.04.13 JHLIM  */
		//if (substream->stream == SNDRV_PCM_STREAM_PLAYBACK)  
		{
			if(wm8993_route_get_playback_path()==HP3P || wm8993_route_get_playback_path()==HP4P	) {
				mx100_wm8993_mute(MUTE_ON,WMS_ANALOGUE_HP | WMS_HPOUT|WMS_MIXOUT);	
			} else if(wm8993_route_get_playback_path()==SPK) {
				mx100_wm8993_mute(MUTE_ON,WMS_MIXOUT |WMS_SPKOUT);
			}

			if (substream->stream == SNDRV_PCM_STREAM_PLAYBACK)  {
				msleep(g_i2s_delay_shutdown_play1);
			} else if(substream->stream == SNDRV_PCM_STREAM_CAPTURE) {
				msleep(g_i2s_delay_shutdown_rec1);
			}		
		}
		#endif
		...
		s5p_i2s_do_suspend(dai);
		if (substream->stream == SNDRV_PCM_STREAM_PLAYBACK)  {
			msleep(g_i2s_delay_shutdown_play2);
		}
		#ifdef REDUCE_I2S_POPS1
		/* 2011.04.13 JHLIM  */
		//if (substream->stream == SNDRV_PCM_STREAM_PLAYBACK)  
		{
			if (substream->stream == SNDRV_PCM_STREAM_PLAYBACK)  {
				...
				msleep(g_i2s_delay_shutdown_rec2);
			}
		
			if(wm8993_route_get_playback_path()==HP3P || wm8993_route_get_playback_path()==HP4P	) {
				mx100_wm8993_mute(MUTE_OFF,WMS_ANALOGUE_HP | WMS_HPOUT|WMS_MIXOUT);	
			} else if(wm8993_route_get_playback_path()==SPK) {
				mx100_wm8993_mute(MUTE_OFF,WMS_MIXOUT |WMS_SPKOUT);
			}
		}
...
static __devinit int s3c64xx_iis_dev_probe(struct platform_device *pdev)
{
	...
	#ifdef USE_DELAY_WORK_SHUTDOWN_I2S
	INIT_DELAYED_WORK(&g_delayed_work_shutdown_i2s, s5p_i2s_set_clk_enabled_delayed_work);
	#endif
...
static __devexit int s3c64xx_iis_dev_remove(struct platform_device *pdev)
{
	...
	mx100_printk("s3c64xx_iis_dev_remove\n");
	mx100_hp_hw_mute(1);
	return 0;
}
