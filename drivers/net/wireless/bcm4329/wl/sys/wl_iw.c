...
wl_control_wl_start(struct net_device *dev)
{
	...
	WL_TRACE(("Enter %s g_onoff=%d \n", __FUNCTION__, g_onoff));
...
)
{
	...
	if (!iw) {
		WL_ERROR(("%s: dev is null\n", __FUNCTION__));
		return -1;
	} else {  /* disable all events */
        char eventmask[WL_EVENTING_MASK_LEN];
        memset(eventmask, 0, WL_EVENTING_MASK_LEN);
        dev_wlc_bufvar_set(dev, "event_msgs", eventmask, WL_EVENTING_MASK_LEN);
        dev_close(dev);
    }
	dhd_os_start_lock(iw->pub);
