...
int sdhci_s3c_get_cd(struct sdhci_host *host)
{
	int rc = ENOSYS;
	struct platform_device *pdev;
	...
	rc =  sdhci_readl(host, SDHCI_PRESENT_STATE) & SDHCI_CARD_PRESENT ;

	printk("sdhci_s3c_get_cd() rc=%d  \n", rc);
...
static int __devinit sdhci_s3c_probe(struct platform_device *pdev)
{
	...
	//host->quirks |= SDHCI_QUIRK_AUTOCMD12;
...
printk("__%s start host->mmc->index = %d \n", __func__, host->mmc->index);
