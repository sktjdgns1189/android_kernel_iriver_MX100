	...
	struct mmc_data *data)
{
	...
	if (host->flags & SDHCI_REQ_USE_DMA)
		mode |= SDHCI_TRNS_DMA;
	if (host->quirks & SDHCI_QUIRK_AUTOCMD12)
...
#ifdef CONFIG_CPU_FREQ_S5PV210
	del_timer_sync(&host->fix_cpufreq_timer);
	if (host->fixing_cpuclock == 0) {
		if (s5pv210_set_cpufreq_level(RESTRICT_TABLE) == RESTRICT_TABLE)
			host->fixing_cpuclock = 1;
	}
#endif
...
/*Shanghai ewada modify for card remove*/
...
/*Shanghai ewada for sd 2011.0409*/
...
static struct mmc_host_ops sdhci_ops = {
	...
	.get_cd		= sdhci_get_cd,
	.adjust_cfg = sdhci_adjust_cfg, /*Shanghai ewada for sd 2011.0409*/
};
...
static void sdhci_tasklet_card(unsigned long param)
{
	struct sdhci_host *host;
	unsigned long flags;
	/*Shanghai ewada for sd death change remove time from 200 to 10 ms*/
	unsigned int ms = 200;
	...
	mmc_detect_change(host->mmc, msecs_to_jiffies(ms));
}
...
static void sdhci_tasklet_finish(unsigned long param)
{
	...
	/*Shanghai ewada modify for sd death*/
	if (NULL == mrq) {
		spin_unlock_irqrestore(&host->lock, flags);
		return;
	}
	 ...
	 * upon error conditions.
	 */
	 /*Shanghai ewada modify for sd death check mrq->cmd*/
	if (!(host->flags & SDHCI_DEVICE_DEAD) &&
...
#ifdef CONFIG_CPU_FREQ_S5PV210
	if (host->fixing_cpuclock == 1)
		mod_timer(&host->fix_cpufreq_timer, jiffies + msecs_to_jiffies(20));
#endif
...
#ifdef CONFIG_CPU_FREQ_S5PV210
static void sdhci_fix_cpufreq_timer(unsigned long data)
{
	struct sdhci_host *host;
	host = (struct sdhci_host*)data;

	if (host->fixing_cpuclock == 1) {
		if (s5pv210_set_cpufreq_level(NORMAL_TABLE) == NORMAL_TABLE)
			host->fixing_cpuclock = 0;
	}
}
#endif
...
int sdhci_add_host(struct sdhci_host *host)
{
	...
	if (host->mmc->caps & MMC_CAP_CLOCK_GATING)
		setup_timer(&host->clock_timer, sdhci_clock_gate_timer,
			    (unsigned long)host);
#ifdef CONFIG_CPU_FREQ_S5PV210
	setup_timer(&host->fix_cpufreq_timer, sdhci_fix_cpufreq_timer,
				(unsigned long)host);
	host->fixing_cpuclock = 0;
#endif
