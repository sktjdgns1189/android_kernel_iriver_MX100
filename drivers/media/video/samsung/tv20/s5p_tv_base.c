...
#ifdef CONFIG_MX100_REV_TP
int mx100_hdmi_cable_in_proc(void)
{
	/* ldo6 regulator on - IO 3.3V*/
	mx100_ldo6_onoff(POWER_ON);
	msleep(10);

	//HDMI PLL Power ON
	s3c_gpio_setpin(S5PV210_GPJ0(3),GPIO_LEVEL_HIGH);
...
int mx100_hdmi_cable_out_proc(void)
{
	//speaker ON
	mx100_hdmi_status = 0;

	mx100_wm8993_power(POWER_ON,WPS_SPK);
	mx100_wm8993_mute(MUTE_OFF, WMS_SPKOUT);

	//HDMI PLL Power OFF
	s3c_gpio_setpin(S5PV210_GPJ0(3),GPIO_LEVEL_LOW);

	/* ldo6 regulator off */
	mx100_ldo6_onoff(POWER_OFF);
...
#ifdef CONFIG_MX100_REV_TP
//[yoon 2011.03.30]HDMI POWER ON
	mx100_hdmi_cable_in_proc();
#endif
...
#ifdef CONFIG_MX100_REV_TP
//[yoon 2011.03.30]HDMI POWER OFF
	mx100_hdmi_cable_out_proc();
#endif
...
#define GPIO_HDMI_ENABLE    S5PV210_GPJ0(3)
#define GPIO_LEVEL_HIGH     1
#define GPIO_LEVEL_LOW      0
  
void iriver_gpio_hdmi_enable(void)
{
	if (gpio_is_valid(GPIO_HDMI_ENABLE)) {
		if (gpio_request(GPIO_HDMI_ENABLE, "GPJ0")) 
			printk(KERN_ERR "Failed to request FM_RESET!\n");
		gpio_direction_output(GPIO_HDMI_ENABLE, GPIO_LEVEL_HIGH);
	}
	
	gpio_free(GPIO_HDMI_ENABLE);	
}
#endif
...
//[yoon 2011.03.30]After Initializatioin, HDMI POWER OFF
	s3c_gpio_setpin(S5PV210_GPJ0(3),GPIO_LEVEL_LOW);
#ifdef CONFIG_MX100_REV_TP
	mx100_ldo6_onoff(0);
#endif
