...
#include "fimc.h"
extern struct mutex                    ctrl_output_lock;              /* controller lock liujianhua*/
			...
			struct v4l2_requestbuffers *b)
{
	...
	if (b->type == V4L2_BUF_TYPE_VIDEO_CAPTURE) {
		...
		mutex_lock(&ctrl_output_lock);
		ret = fimc_reqbufs_output(fh, b);
		mutex_unlock(&ctrl_output_lock);
	} else {
...
long fimc_default_ioctl(struct file *file, void *fh,
					int cmd, void *arg)
{
	int rc = -1;
	printk("fimc_default_ioctl cmd=%x VIDIOC_PRIVATE_SET_CAM_ID=%d \n ", cmd, VIDIOC_PRIVATE_SET_CAM_ID);
	switch (cmd) {
	
		case VIDIOC_PRIVATE_SET_CAM_ID:
			rc =fimc_set_cam_id(file, fh, cmd, arg);
			break;
		case VIDIOC_PRIVATE_GET_CAM_ID:
			break;
		default:
			return rc;
			break;
...
const struct v4l2_ioctl_ops fimc_v4l2_ops = {
	...
	.vidioc_default = fimc_default_ioctl,
};
