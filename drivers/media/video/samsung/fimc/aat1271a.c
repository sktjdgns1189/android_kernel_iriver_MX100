...
Address  EN/SET  Rising Edges Function  				Default  (No programming)
==========================================================
0 		17 		Movie Mode Current 				100%
1 		18 		Flash Safety Timer 				16/16
2 		19 		Movie Mode Output Confi guration 	FLOUTA and FLOUTB ON
3 		20 		Flash/Movie Mode Current Ratio 	1/7.3
*/
...
#define GPIO_CAM_FLASH_SET GPIO_CAM_FLASH_SET_OUT
#define GPIO_CAM_FLASH_EN GPIO_CAM_FLASH_EN_OUT

#define S3C_GPIO_LAVEL "GPD0"

static int g_movie_movie_current = MOVIE_MODE_CURRENT_63;
...
static int __init aat1271a_flash_init(void)
{	
	...
	if (gpio_is_valid(GPIO_CAM_FLASH_SET)) {
		if (gpio_request(GPIO_CAM_FLASH_SET, S3C_GPIO_LAVEL)) 
	...
	if (gpio_is_valid(GPIO_CAM_FLASH_EN)) {
		if (gpio_request(GPIO_CAM_FLASH_EN, S3C_GPIO_LAVEL)) 
	...
	printk("--------------------------------------------------------\n");
	printk("aat1271a flash led driver initialized\n");
	printk("--------------------------------------------------------\n");
...
static void __exit aat1271a_flash_exit(void)
{
...
module_init(aat1271a_flash_init);
module_exit(aat1271a_flash_exit);

MODULE_AUTHOR("Iriver");
MODULE_LICENSE("GPL");
MODULE_DESCRIPTION("AAT1271a flash led driver for mx100");
...
{
	aat1271a_flash_write(addr,data);	
}

void mx100_aat1271a_flash_en(int value)
{
	gpio_set_value(GPIO_CAM_FLASH_EN, value);
}

void mx100_aat1271a_flash_set(int value)
{
	gpio_set_value(GPIO_CAM_FLASH_SET, value);
}

void mx100_aat1271a_camera_control(int value)
{
		aat1271a_falsh_camera_control(value);
}
