...
struct mutex			ctrl_output_lock;		/* controller lock liujianhua*/
static int lock_enter_times = 0;
...
static int fimc_open(struct file *filp)
{
	...
	if (in_use >= FIMC_MAX_CTXS || (in_use && 1 != ctrl->id)) {
		printk("fimc_open ctrl->id: %d is busy, in_use: %d\n",ctrl->id,in_use);
		ret = -EBUSY;
		goto resource_busy;
	} else {
...
static int fimc_release(struct file *filp)
{
	...
	if (ctrl->cam && (ctrl->cam->initialized == 1) && ctrl->id != 2) {
		/* shutdown the MCLK */
		...
		{
			ctrl->cam->cam_power(0, ctrl->cam->clk);
			camera_id = 0; //reset camera ID as back camera when release camera
		}
	...
	if (ctrl->out) {
		ctx = &ctrl->out->ctx[ctx_id];  //liujianhua add for test
...
static int fimc_init_global(struct platform_device *pdev)
{
	...
	for (i = 0; i < FIMC_MAXCAMS; i++) {
		...
		fimc_dev->camera[cam->id] = cam;
		printk("cam id=%d name=%s \n", cam->id, cam->info->type);
	}
...
static int fimc_get_camera_id(struct device *dev,
		struct device_attribute *attr, char *buf)
{
	if (camera_id == 0)
		sprintf(buf, "%s", "back");
	...
	//printk("get camera_id:%s\n", buf);
	return strlen(buf);
}

static int fimc_set_camera_id(struct device *dev,
		struct device_attribute *attr, const char *buf, size_t len)
{
	if (strcmp(buf, "front") == 0)
		...
		camera_id = 0;
	printk("set camera_id:%d\n", camera_id);

	return len;
}

static DEVICE_ATTR(camera_id, 0666, \
			fimc_get_camera_id,
			fimc_set_camera_id);
...
static int __devinit fimc_probe(struct platform_device *pdev)
{
	...
	//printk(KERN_INFO "FIMC%d registered successfully\n", ctrl->id);
	if(lock_enter_times == 0)
	{
		mutex_init(&ctrl_output_lock);
	}
	lock_enter_times++;
	return 0;
...
static int fimc_remove(struct platform_device *pdev)
{
	...
	fimc_dev = NULL;
		lock_enter_times--;
	if(lock_enter_times ==0)
		mutex_destroy(&ctrl_output_lock);
	return 0;
}
