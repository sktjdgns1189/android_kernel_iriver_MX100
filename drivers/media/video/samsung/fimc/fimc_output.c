...
int fimc_qbuf_output(void *fh, struct v4l2_buffer *b)
{
	...
	if (ctx->src[b->index].state != VIDEOBUF_IDLE) {
		fimc_err("The index(%d) buffer must be dequeued state(%d):ctrl[%d],ctx[%d]\n",
				 b->index, ctx->src[b->index].state,ctrl->id,ctx_id);
		return -EINVAL;
	}
...
void fimc_dump_context(struct fimc_control *ctrl, struct fimc_ctx *ctx)
{
	...
	fimc_err("ctrl->id: %d, ctx%d, ctrl->status: %d, ctx->status: %d\n",
		ctrl->id, ctx->ctx_num, ctrl->status, ctx->status);
