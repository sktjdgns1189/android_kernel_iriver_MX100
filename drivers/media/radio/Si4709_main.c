...
#ifdef USE_FM_IRQ
/*Si4709 IRQ Number*/
#define FM_IRQ_INT	S3C_GPIOINT(J2,4)
#endif	
...
extern void mx100_hw_mute(int mute);

#ifdef CONFIG_I2S_FM_FEED_MCLK	
extern void enable_i2s_fm_mclk(void);
extern void disable_i2s_fm_mclk(void);
#endif

int g_fm_on = 0;  // JHLIM 2011.04.12



int get_fm_output(void)
{
	return	fmdata.output;
}

void fm_start_for_codec(void)
{
	fm_start(&fmdata);
}
				...
				unsigned int ioctl_cmd,  unsigned long arg)
{
	...
	{
/* ktj start */
		case Si4709_IOC_FM_START:
			{
				fmdata_t _fmdata;
				unsigned long n;
				debug("Si4709_IOC_FM_START called");
				
//				audio_control_lock();

				n = copy_from_user((void*) &_fmdata, argp, sizeof(fmdata_t));
				
				if(n) {
					debug("Si4709_IOC_FM_START() : copy_from_user() has error!!Failed to read [%d] byes!", n);
					ret = -EFAULT;
				} 

				fmdata.freq = _fmdata.freq;
				fmdata.band = _fmdata.band;
				#ifdef SI4702_VOLUME_CODEC	
				fmdata.volume = 10;  //default fm volume adjust.
				#else
				fmdata.volume = _fmdata.volume;
				#endif
				
				fmdata.output = _fmdata.output;
				mx100_printk(" fm start start:%d!!! \n",fmdata.output);
...
//				fm_start(&fmdata);
	//			msleep(100);
				...
				set_autio_codec_path(PATH_FMRADIO,FMR_ON);
				
//				audio_control_unlock();

				mx100_printk(" fm start end!!!\n");
			}
			break;

		case Si4709_IOC_FM_TUNE:
			{
				fmdata_t _fmdata;
				unsigned long n;
				debug("Si4709_IOC_FM_TUNE called");
				n = copy_from_user((void*) &_fmdata, argp, sizeof(fmdata_t));
				
				if(n) {
					debug("Si4709_IOC_FM_TUNE() : copy_from_user() has error!!Failed to read [%d] byes!", n);
					ret = -EFAULT;
				} 
		...
		case Si4709_IOC_FM_VOLUME:
			{
				...
				if(n) {
					debug("Si4709_IOC_FM_VOLUME() : copy_from_user() has error!!Failed to read [%d] byes!", n);
					ret = -EFAULT;
				} 
		...
		#ifdef SI4702_VOLUME_CODEC	
		case Si4709_IOC_FM_VOLUME_TUNING:
			{
				fmdata_t _fmdata;
				unsigned long n;
				debug("Si4709_IOC_FM_TUNE called");
		...
		case Si4709_IOC_FM_SEEK:
			{
				fmdata_t _fmdata;
				unsigned long n;
				debug("Si4709_IOC_FM_SEEK called");

				n = copy_from_user((void*) &_fmdata, argp, sizeof(fmdata_t));
				
				if(n) {
					debug("Si4709_IOC_FM_SEEK() : copy_from_user() has error!!Failed to read [%d] byes!", n);
					ret = -EFAULT;
				} 

                fmdata.freq = _fmdata.freq;
                fmdata.seek = 1;
                fmdata.seekmode = _fmdata.seekmode;
                fmdata.seekdir = _fmdata.seekdir;
				
				fm_scan(&fmdata);

                fmdata.seek = 0;

                if( copy_to_user(argp, (void*) &fmdata, sizeof(fmdata_t))) {
                    debug("Si4709_IOC_FM_SEEK() : copy_to_user() has error!\n");   
					ret = -EFAULT;			
			    }
			}
			break;

		case Si4709_IOC_FM_OUTPUT:
			{
				fmdata_t _fmdata;
				unsigned long n;
				debug("Si4709_IOC_FM_TUNE called");

				n = copy_from_user((void*) &_fmdata, argp, sizeof(fmdata_t)); /* remove warning */
				
				if(n) {
					debug("Si4709_IOC_FM_OUTPUT() : copy_from_user() has error!!Failed to read [%d] byes!", n);
					ret = -EFAULT;
				} 
		...
		case Si4709_IOC_FM_HPCONNECT:
			debug("Si4709_IOC_FM_HPCONNECT called");
			
			fm_hpconnect(&fmdata);
			
           		 if( copy_to_user(argp, (void*) &fmdata, sizeof(fmdata_t))) {
                		debug("Si4709_IOC_FM_HPCONNECT() : copy_to_user() has error!\n");   
				ret = -EFAULT;			
			}
			break;

		case Si4709_IOC_FM_STATUS:
			debug("Si4709_IOC_FM_STATUS called");
			
			fm_status(&fmdata);
			
            if( copy_to_user(argp, (void*) &fmdata, sizeof(fmdata_t))) {
                debug("Si4709_IOC_FM_STATUS() : copy_to_user() has error!\n");   
				ret = -EFAULT;			
			}
			break;

		case Si4709_IOC_FM_STOP:
			debug("Si4709_IOC_FM_STOP called");      
//			audio_control_lock();

			mx100_printk(" fm stop start!!!\n");
			/* JHL  */
			g_fm_on = 0;

			set_autio_codec_path(PATH_FMRADIO,FMR_OFF);

			mx100_printk(" fm stop end!!! \n");
		...
		case Si4709_IOC_SEEK_SNR_SET:
			{
				...
				set_fm_seeksnr(seek_SNR_th);  /* JHLIM */
			}
			break;
 ...
 int __init Si4709_driver_init(void)
{
   ...
   /*KGVS: Configuring the GPIO_FM_INT in mach-jupiter.c*/
	s3c_gpio_cfgpin(GPIO_FM_INT, S3C_GPIO_SFN(GPIO_INPUT));
