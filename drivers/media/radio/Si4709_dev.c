...
#define SEEK_SNR        0      /*JHLIM 2011.04.26  seek snr_threshhold */
#define SEEK_CNT        0  

static int g_seek_snr = SEEK_SNR;  
static int g_seek_cnt = SEEK_CNT;  
...
/**********************************************/
#ifdef IRIVER_FM_MODIFY_CLK
static void enable_clk(int enable);
#endif
...
int Si4709_dev_init(struct i2c_client *client)
{
	...
	ret = gpio_request(FM_RESET, "fm reset");
	if (ret)
	{
		printk("  Si4709_dev_init  gpio_request failed \n");
	}
    ...
    gpio_set_value(FM_RESET, GPIO_LEVEL_LOW); /* ktj keep low */	
...
int Si4709_dev_cur_RSSI_get(rssi_snr_t *cur_RSSI)
{
    ...
    else
    {
        ...
        else
        {
	     ...
	     cur_RSSI->curr_skcnt = g_seek_cnt;
        }
    }     
    mutex_unlock(&(Si4709_dev.lock)); 
...
#ifdef IRIVER_RADIO
    debug("part_number=0x%x manufact_number=0x%x\n", dev_id->part_number, dev_id->manufact_number);
#endif
...
#ifdef IRIVER_RADIO
    debug("chip_version=0x%x device=0x%x firmware_version=0x%x\n", chp_id->chip_version, chp_id->device, chp_id->firmware_version);
#endif
...
static int powerdown(void)
{
	...
	if(!(RADIO_POWERDOWN==Si4709_dev.state.power_state))
	{
		...
		else
		{
		 ...
		 /****Resetting the device****/	
		gpio_set_value(FM_RESET, GPIO_LEVEL_LOW);
//		gpio_set_value(FM_RESET, GPIO_LEVEL_HIGH);  /* ktj disbale , keep low */ 
        s3c_rtc_clkout(0); /* ktj */
#ifdef IRIVER_FM_MODIFY_CLK
		enable_clk(0);
#endif
	}
...
-----------------------------------------------------------------------*/
#ifdef SI4702_VOLUME
#define FM_DATA_NUM     10
#else
//#define FM_DATA_NUM     8
#define FM_DATA_NUM     10  /* 2011.04.26 JHLIM  */
#endif

//#define SEEK_TH         23      /* seek threshhold */
#define SEEK_TH         13      /* modify 2011.04.28 JHLIM  */
#define SEEK_DELAY_MS	150     /* before 100 */
...
/*
typedef enum {
    SEEK_RESULT_SUCCESS = 0,        
    SEEK_RESULT_LEVEL_LOW,      
    SEEK_RESULT_FAIL,           
    SEEK_RESULT_TIMEOUT        
} SEEK_RESULT_TYPE;
*/
...
static int DivUnit;
static int BaseFreq; 
static int seekth = SEEK_TH;  
static int gFmPower = 0;
#ifndef SI4702_VOLUME
static int fmvolume = 0;    /* 0~15 */
#endif


fmdata_t fmdata;
void set_fm_seekth(int _seekth)  /* JHLIM 2011.04.26 */
{
seekth = _seekth;
}

void set_fm_seeksnr(int _seeksnr)  
{
	g_seek_snr = _seeksnr;
}

void set_fm_seekcnt(int _seekcnt)  
{
	g_seek_cnt = _seekcnt;
}
...
/*---------------------------------------------------------------------*/
#ifndef SI4702_VOLUME
int fm_volume(int volume)
{
    /* 
        TODO : 
        LJH make line-in volume control function
    */

    debug("set fm volume = %d\n", volume);

    return 0;
}
#endif

void fm_volume_tuning(int vol)
{
        fmdata.volume = vol;
	fmdata.volcont = 1;
	fm_tune(&fmdata);
...
int pllout(fmdata_t *fmdata)
{
	int ret;
    int msglen = FM_DATA_NUM;
    unsigned char RadioData[FM_DATA_NUM];
#ifdef SI4702_VOLUME
    int vol_reg, vol_ext;
#endif
...
#ifndef SI4702_VOLUME
    if (fmvolume != fmdata->volume)
    {
        fmvolume =  fmdata->volume;
        fm_volume(fmvolume);  
    }
#endif
	
	memset(RadioData, 0, FM_DATA_NUM);	


	// Set band_limit, step, volume, de-emphasis
	if (fmdata->band == BAND_JAPAN)
	{
		RadioData[7]    = 0x5f;	// band limit : 76~108, Step : 100kHz, Volume : 16
		RadioData[4]    = 0x08;	// de-emphasis : 50us
		DivUnit         = 100;
		BaseFreq        = 76000;       //khz unit
	}
	else if (fmdata->band == BAND_EU)
	{
		RadioData[7] = 0x2f;	// band limit : 87.5~108, Setp : 50kHz, Volume : 16
		RadioData[4] = 0x08;	// de-emphasis : 50us
		DivUnit = 50;
		BaseFreq = 87500;       // kHz unit
	}
	else
	{
		RadioData[7] = 0x1f;	// band limit : 87.5~108, Step : 100kHz, Volume : 16
		RadioData[4] = 0x00;	// de-emphasis : 75us
		DivUnit = 100;
		BaseFreq = 87500; // khz unit 
	}
    ...
    // Set stereo or mono
	if (fmdata->stereo == FM_MONO)
		RadioData[0] |= 0x20;	// forced mono, set adc equal 5?
	else
		RadioData[0] |= 0x00;	// stereo, set adc equal 7?


	// Standby mode setting
	if (fmdata->standby == 1) 
	{
		RadioData[1] = 0x40;	// tuner standby set
		RadioData[0] &= ~0x40;	// audio mute set
	}
	else
	{
		RadioData[1] = 0x01;	// tuner active
		RadioData[0] |= 0x40;	// audio mute release
	}


	// Frequency setting
	fmdata->freq = (fmdata->freq - BaseFreq)/DivUnit; //kHz Unit
	RadioData[2] |= (unsigned char)((fmdata->freq >> 8) & 0x03);
	RadioData[3] = (unsigned char)(fmdata->freq & 0xff);
    ...
    if (fmdata->volume > 15)
    {
        ...
        vol_ext = 1;
        vol_reg = fmdata->volume - 15;
    }
    else
    {
        vol_ext = 0;   
        vol_reg = fmdata->volume;
    }

    debug("volume = %d, vol_reg = %d vol_ext = %d\n", fmdata->volume, vol_reg, vol_ext);

    RadioData[7] &= 0xf0;               // volume set 0 
    RadioData[7] |= vol_reg & 0x0f;
    RadioData[8] |= vol_ext & 0x01;

    if (fmdata->volcont == 0)
    {
	    RadioData[0] &= (~0x40);  // mute enable
	}
	else
	{
	    RadioData[0] |= (0x40);   // mute disable
	}
#endif
	...
	// Tune Clear
#ifndef SI4702_VOLUME
	RadioData[0] &= (~0x40);  // mute set
#endif
	RadioData[2] &= ~0x80;		

    // 반드시 해야함, 아래 과정 빠지면 튜닝 안됨
    ret = i2c_master_send((struct i2c_client*)(Si4709_dev.client), ( const char *)RadioData, msglen);
    ...
    {
	    // Set mute
	    if (!fmdata->mute) RadioData[0] |= (0x40);            
        
        // Set seek
	    if (fmdata->seek)
	    {
	    	RadioData[0] |= 0x01;
	    	RadioData[0] |= (fmdata->seekdir << 1) & 0x02;
	    	RadioData[0] |= (fmdata->seekmode << 2) & 0x04;
	    	RadioData[6] |= seekth;
	    	fmdata->seek = 0;
		RadioData[8] |= (g_seek_snr & 0xf) << 4;  /* JHLIM 2011.04.26 */	
		RadioData[8] |= (g_seek_cnt & 0xf);  
	    }
	  /* JHLIM    0:power con 1:2channel 3:4 syscon1 5:6syscon2 */
...
int pllread(void)
{
	int ret;
	int msglen = 4;
   	unsigned char RadioData[4];

	memset(RadioData, 0xEE, 4);
    ...
    */
    ...
    ret = i2c_master_recv((struct i2c_client*)(Si4709_dev.client), RadioData, msglen);

	{
		unsigned int readchan, re_freq, stereo, rssi, stc;
		

		readchan = (((RadioData[2] & 0x03) << 8) | RadioData[3]);
		re_freq = ((readchan * DivUnit) + BaseFreq);
		stereo = RadioData[0] & 0x01;
		rssi = RadioData[1];
		stc = (RadioData[0] & 0x40) >> 6;
		
    	debug("------ pllread called  ----------------------");
    	debug("D[0]=0x%x D[1]=0x%x D[2]=0x%x D[3]=0x%x", RadioData[0], RadioData[1], RadioData[2], RadioData[3]);
		debug("freq=%d stc=%d stereo=%d rssi=%d", re_freq, stc, stereo, rssi);
    	debug("----------------------------------------------");
	}

	return (RadioData[0] & 0x01);	
}
...
int fm_init(void)
{
    ...
    fmdata.freq         = 89100;
    fmdata.band         = BAND_KOREA;
    fmdata.volume       = 15;
    fmdata.volcont      = 0;
    fmdata.output       = OUTPUT_HEADPHONE;
    fmdata.hpconnect    = -1;

    fmdata.seek         = 0;
    fmdata.seekmode     = SEEK_MODE_TUNE;
    fmdata.seekdir      = SEEK_DIR_UP;
    fmdata.seekresult   = SEEK_SUCCESS;

    fmdata.stereo       = FM_STEREO;
    fmdata.mute         = 0;
    fmdata.standby      = 0;
    fmdata.status       = 0;
...
int fm_start(fmdata_t *fmdata)
{
	int ret = 0;

    debug("%s called freq = %d",__func__, fmdata->freq);
	...
	/*Shanghai ewada modify for fm path*/
   
	//wm8993_setpath_fm_route(1);
...
debug("___volume = %d", fmdata->volume);

    mutex_lock(&(Si4709_dev.lock));   

//	seek = 1;
//  pllout(client, freq, 1);	// mute on and tuning
    fmdata->seek = 1;
    fmdata->mute = 1;
    pllout(fmdata);
...
debug("___volume = %d", fmdata->volume);
	...
	msleep(SEEK_DELAY_MS);	    // wating for tuning

//	fm_read(client, &RadioData[0], 4);
    ret = i2c_master_recv((struct i2c_client*)(Si4709_dev.client), RadioData, 4);

	if((RadioData[0] & 0x40) == 0)				        // STC[6] : indicates seek/tune complete
    {
		debug("error 1 time : seek not complete\n");
    	msleep(SEEK_DELAY_MS);	                        // wating for tuning
    	ret = i2c_master_recv((struct i2c_client*)(Si4709_dev.client), RadioData, 4);
    }
...
//  ret = re_freq;
	...
	if(RadioData[0] & 0x40)			                    // STC[6] : indicates seek/tune complete
	{
		if((RadioData[0] & 0x20) == 0)		            // SF/BL[5] success=0, fail = 1
		{
		    if(RadioData[1] >= seekth)                  // RSSI[7:0]
			{
        		debug("seek success freq : %d\n", re_freq);
			    fmdata->seekresult = SEEK_SUCCESS;
			}
			else
			{
    			debug("error : seek level is low  cur_seekth=%d limit_seekth=%d\n", RadioData[1], seekth);
			    fmdata->seekresult = SEEK_LEVEL_LOW;
			}
		}
		else
		{
			debug("error : seek fail\n");
			fmdata->seekresult = SEEK_FAIL;
		}	
	}
	else
	{
		debug("error 2 time : seek not complete\n");
		fmdata->seekresult = SEEK_TIMEOUT;
	}

debug("___volume = %d", fmdata->volume);

    if (fmdata->seekmode == 0 && fmdata->seekresult == SEEK_SUCCESS)
    {    
        //pllout(client, re_freq, 0);
        fmdata->mute = 0;   // 여기서 풀어줘야 함
        pllout(fmdata);
    
        debug("seeked freq : %d\n", re_freq);
    }

    fmdata->freq = re_freq; // pllout 에서 값 변하므로 reloading

    mutex_unlock(&(Si4709_dev.lock));   	 

    //return (0x00ffffff & ret | result<<24);
    return 0;
}

int fm_output(fmdata_t *fmdata)
{
extern int g_fm_volume;

    debug("fmdata->output = %d\n", fmdata->output);    

    mutex_lock(&(Si4709_dev.lock));   

     mx100_printk(" fm out %d %d!!! \n",fmdata->output,g_fm_volume);
/* 2011.05.18 JHLIM  */
	if(fmdata->output ==0) {  // mute
...
/*
    if (fmdata->output)
        wm8993_setpath_speaker(1);    
    else
        wm8993_setpath_speaker(0);    
*/
...
int fm_hpconnect(fmdata_t *fmdata)
{
    mutex_lock(&(Si4709_dev.lock));   

    if (g_headset_connected)
        fmdata->hpconnect = 1;
    else
        fmdata->hpconnect = 0;
...
int fm_stop(void)
{
    ...
    ret = i2c_master_send((struct i2c_client*)(Si4709_dev.client), ( const char *)RadioData, 4);

    gpio_set_value(FM_RESET, GPIO_LEVEL_LOW);   /* reset low */	
...
int fm_test(void)
{
    ...
    Si4709_dev_device_id(&dev_id);

    Si4709_dev_chip_id(&chp_id);
    ...
    Si4709_dev_chan_get(&frequency);

//  Si4709_dev_powerdown();
...
#define S3C_VA_RTC_CON	S3C_ADDR(0x02800000)	/* system control */
#define S5P_RTC_CLKREG(x)		(S3C_VA_RTC + (x))

#define S5P_RTC_CON		S5P_RTC_CLKREG(0x40)
...
static void enable_clk(int enable)
{
	...
	//PAGE 786 rtccob 0x0xe2800040
	pclk = clk_get(&Si4709_dev.client->dev, "rtc");

	if (IS_ERR(pclk)) 
	{
		printk("failed to get interface clock\n");
	}
	else
	{
		clk_enable(pclk);

		writel(readl(S5P_RTC_CON)|(1<<9),S5P_RTC_CON);
		
		printk("............ %s() line %d REG=%x \n", __FUNCTION__, __LINE__, readl(S5P_RTC_CON));
	}
#endif
