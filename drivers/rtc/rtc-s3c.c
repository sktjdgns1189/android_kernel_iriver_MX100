...
/* ktj */
//----------------------------------------------------------------------------------
//  #define rdebug(fmt,arg...) printk(KERN_CRIT "___[rtc-s3c] " fmt "\n",## arg)
...
#ifdef IRIVER_WAKEUP_TEST   /* ktj */
    tm->tm_sec =  rtc_temp->tm_sec + INTERVAL_SEC;
    tm->tm_min =  rtc_temp->tm_min;
    tm->tm_hour = rtc_temp->tm_hour;
...
#if defined(CONFIG_MX100) /* ktj, rtc clockout for fm tuner */
void s3c_rtc_clkout(int en)
{
	void __iomem *base = s3c_rtc_base;
	unsigned int tmp;
	...
	{
        rdebug("error : s3c_rtc_base is NULL");
		return;
    }
...
static int __devinit s3c_rtc_probe(struct platform_device *pdev)
{
	...
	/* check rtc time */
#if defined(CONFIG_MX100) /* ktj init rtc time */
	rtc_invalid_date = 0;
	...
	for (bcd_loop = S3C2410_RTCSEC ; bcd_loop <= S3C2410_RTCYEAR ; bcd_loop +=0x4)
	{
        ...
        {
            printk("[rtc] rtc_invalid_date\n"); 
            rtc_invalid_date = 1;
            break;	            
	    }
	}
        if(strcmp(g_mx100_mp_mode, "process") == 0)   // jhlim 2011.07.19 
	{
		rtc_invalid_date = 1;
	}
	...
	if (rtc_invalid_date) 
	{
		...
		tm.tm_year = 11;
       	...
       	tmp = readw(s3c_rtc_base + S3C2410_RTCCON);
       	writew(tmp|S3C2410_RTCCON_RTCEN, s3c_rtc_base+S3C2410_RTCCON);

		writeb(s3c_bin2bcd(tm.tm_sec),  s3c_rtc_base + S3C2410_RTCSEC); 
		writeb(s3c_bin2bcd(tm.tm_min),  s3c_rtc_base + S3C2410_RTCMIN); 
		writeb(s3c_bin2bcd(tm.tm_hour), s3c_rtc_base + S3C2410_RTCHOUR); 
		writeb(s3c_bin2bcd(tm.tm_mday), s3c_rtc_base + S3C2410_RTCDATE); 
		writeb(s3c_bin2bcd(tm.tm_mon+1), s3c_rtc_base + S3C2410_RTCMON); 

        year = tm.tm_year + 100 + 300;
       	year = (0x00000fff & year);
       	writel(s3c_bin2bcd(year), s3c_rtc_base + S3C2410_RTCYEAR);

     	writew(tmp, s3c_rtc_base+S3C2410_RTCCON);

        printk("[rtc] set default 2011-01-01 09:00\n"); 
    }
#else
	for (bcd_loop = S3C2410_RTCSEC ; bcd_loop <= S3C2410_RTCYEAR ; bcd_loop +=0x4)
	{
