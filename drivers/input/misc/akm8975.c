...
#include "akm8975.h"    // <linux/akm8975.h> /* ktj */
...
#define AKMDBG(format, ...)	printk(KERN_INFO "___[akm8975] " format "\n", ## __VA_ARGS__) 
#else
#define AKMDBG(format, ...)
#endif

#if AKM8975_DEBUG_FUNC
#define AKMFUNC(func) printk(KERN_INFO "___[akm8975] " func " is called\n")
#else
#define AKMFUNC(func)
#endif
...
int akm8975_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	/* yaw (0, 360) */
	input_set_abs_params(akm->input_dev, ABS_RX, -11520, 11520, 0, 0);
	/* pitch (-180, 180) */
	input_set_abs_params(akm->input_dev, ABS_RY, 0, 23040, 0, 0);
	/* roll (-90, 90) */
...
MODULE_DEVICE_TABLE(i2c, akm8975_id); /* ktj */
...
static int __init akm8975_init(void)
{
    ...
    if ( (ret = i2c_add_driver(&akm8975_driver) < 0) ) 
    {
        printk("AKM8975 i2c_add_driver failed");
    }

	printk(KERN_INFO "akm8975_init done\n");
