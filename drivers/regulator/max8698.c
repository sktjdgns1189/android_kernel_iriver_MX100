...
static u8 max8698_cache_regs[16] = {
	0xFE, 0xF1, 0xFF, 0xF9, // all blocks on
	0x99, 0x99, 0x77, 0x02,
	0x66, 0x11, 0x0C, 0x11,
	0x02, 0x33, 0x0E, 0x16,
};
...
//[yoon 2011.03.30]HDMI LDO6 POWER ON/OFF
int mx100_ldo6_onoff(int onoff)
{
	struct regulator *dac_regulator;
	unsigned char reg_val;

	dac_regulator = regulator_get(NULL, "vddlcd");
				...
				int *_reg, int *_shift, int *_mask)
{
	...
	case MAX8698_LDO2 ... MAX8698_LDO3:
		...
		mask = 0xf;
		if (ldo == MAX8698_LDO2)
			shift = 0;	// modified to 4 from 0 by sangwon
		else
			shift = 4;  // modified to 0 from 4 by sangwon
		break;
				...
				const struct i2c_device_id *i2c_id)
{
	...
	/* Shanghai ewada: update local register values */
	size = sizeof(max8698_cache_regs)/sizeof(max8698_cache_regs[0]);
	for (i = 0; i < size ; i++) {
		max8698_i2c_read(client, i, &val);
	}
...
#if defined(CONFIG_MX100) /* ktj */
void pmic_write_reg(unsigned char reg, unsigned char value)
{
	max8698_i2c_write(pmic_client, reg, value);
    printk("[%s] reg = 0x%02x value = 0x%02x\n", __func__, reg, value);
}

unsigned char pmic_read_reg(unsigned char reg)
{
	unsigned char value;
    max8698_i2c_read(pmic_client, reg, &value);
    printk("[%s] reg = 0x%02x value = 0x%02x\n", __func__, reg, value);
    return value;
}

/*Shanghai ewada for poweroff*/
void max_8698_poweroff() {
	printk(KERN_EMERG "%s : setting regs to poweroff \n", __func__);
	max8698_i2c_write(pmic_client, 3, 0x09); /*ldo7 8 off */
	max8698_i2c_write(pmic_client, 1, 0x10); /*ldo9 on */
	max8698_i2c_write(pmic_client, 2, 0x30); /*ldo2 on buck3 on*/
}
