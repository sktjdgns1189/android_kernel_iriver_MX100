...
static struct regulator *lcd_mod_regulator;
#endif

#if defined(CONFIG_MX100) /* ktj */
/*---------------------------------------------------------------------*/
/* ktj, dump display controller register */
//#define DUMP_DISP_REG
//#define DUMP_GPIO_REG

extern void dump_gpio_register(void);

#ifdef DUMP_DISP_REG    /* ktj */
#define VID_BASE_ADDR   0xF8000000
#define VID_END_ADDR    0xf800037c    
int s3cfb_dump_register(struct s3cfb_global *ctrl)
{
	__u32 cfg;
    unsigned long i = 0;
    ...
    {
        cfg = readl(ctrl->regs + i);
        printk("[vid_reg] 0x%x 0x%x = 0x%8x \n", ctrl->regs+i, VID_BASE_ADDR+i, cfg);
    }
...
#if defined(CONFIG_MX100) /* ktj */
extern int s3cfb_set_reg(struct s3cfb_global *ctrl, u32 reg, u32 data);
extern int s3cfb_get_reg(struct s3cfb_global *ctrl, u32 reg);
extern int s3cfb_set_gammalut(struct s3cfb_global *ctrl, u32 mode);
extern int s3cfb_get_gammalut(struct s3cfb_global *ctrl, u32 mode);

void set_reg(u32 reg, u32 data)
{
    s3cfb_set_reg(fbdev, reg, data);
}
EXPORT_SYMBOL(set_reg);

void get_reg(u32 reg)
{
    s3cfb_get_reg(fbdev, reg);
}
EXPORT_SYMBOL(get_reg);

void set_gammalut(u32 mode)
{
    s3cfb_set_gammalut(fbdev,mode);
}
EXPORT_SYMBOL(set_gammalut);

void get_gammalut(u32 mode)
{
    s3cfb_get_gammalut(fbdev,mode);
}
EXPORT_SYMBOL(get_gammalut);
#endif
...
#include "booting_logo_iriver.h"
#define	IRIVER_LOGO_WIDTH		190
#define	IRIVER_LOGO_HEIGHT	280
#define	LOGO_OFFSET_XPOS		410
#define	LOGO_OFFSET_YPOS		150

static int s3cfb_mx100_draw_logo(struct fb_info *fb)
{
    struct s3c_platform_fb *pdata = to_fb_plat(fbdev->dev);
    struct fb_fix_screeninfo *fix = &fb->fix;
    struct fb_var_screeninfo *var = &fb->var;
    int logo_width = IRIVER_LOGO_WIDTH;
    int logo_height = IRIVER_LOGO_HEIGHT;
    int x_offset = LOGO_OFFSET_XPOS;	//(var->xres - logo_width)/2;
    int y_offset = LOGO_OFFSET_YPOS;	//(var->yres - logo_height)/2;
    int y;
	unsigned int * LOGO;
	int charging_mode;
	int logo_size;
	...
	for(y = 0; y < logo_height; y++)  {
		memcpy(fbdev->fb[pdata->default_win]->screen_base + fix->line_length * (y+y_offset) + x_offset * (var->bits_per_pixel / 8),
...
#if defined(CONFIG_MX100) /* ktj set gamma */
    s3cfb_set_gammalut(fbdev, 8);
//  s3cfb_set_reg(fbdev, 448, 277091632);
    s3cfb_set_reg(fbdev, 448, 0x0F73AD00);  // 2011-03-23 changed
    s3cfb_set_reg(fbdev, 12, 327680);
#endif
...
#if defined(CONFIG_MX100)
//[yoon]MX100: draw boot logo
			s3cfb_mx100_draw_logo(fbdev->fb[j]);
#else
...
static int s3cfb_probe(struct platform_device *pdev)
{
...
#if defined(CONFIG_MX100) /* ktj_pm */
	/* ldo4 regulator on */
	lcd_mod_regulator = regulator_get(NULL, "vddlcd_mod");
	if (IS_ERR(lcd_mod_regulator)) {
		printk(KERN_ERR "failed to get resource %s\n", "vddlcd_mod");
		return PTR_ERR(lcd_mod_regulator);
	}
	regulator_enable(lcd_mod_regulator);
#endif
	...
	if (pdata->backlight_on)
		pdata->backlight_on(pdev);
#elif defined(CONFIG_FB_S3C_LTE480WV) || defined(CONFIG_FB_S3C_LMS700) || defined(CONFIG_FB_S3C_LD070)
	if (pdata->backlight_on)
		pdata->backlight_on(pdev);
#endif
...
#if defined(CONFIG_MX100) /* ktj_pm */
	/* ldo4 regulator off */
	regulator_disable(lcd_mod_regulator);
	regulator_put(lcd_mod_regulator);
#endif
...
#if defined(CONFIG_FB_S3C_LTE480WV) || defined(CONFIG_FB_S3C_LMS700) || defined(CONFIG_FB_S3C_LD070)
	if (pdata->backlight_off)
		pdata->backlight_off(pdev);
#endif
...
#if defined(CONFIG_MX100) /* ktj */
    gpio_set_value(GPIO_LCD_CS,  GPIO_LEVEL_LOW);
    gpio_set_value(GPIO_LCD_CLK, GPIO_LEVEL_LOW);
    gpio_set_value(GPIO_LCD_SI,  GPIO_LEVEL_LOW);
#endif
...
*/
	...
	if (pdata->backlight_on)
	{
		msleep_interruptible(100);  // ktj add delay 
		pdata->backlight_on(pdev);
    }
